...
var cloudwatchlogs = new CloudWatchLogs({apiVersion: '2014-03-28', region: process.env.AWS_REGION});
...
let getSlowlogs = async (client, maxAge, slowlogGetLimit) => {
  return new Promise((resolve, reject) => { // promise wrapper needed because node-redis does not support promises natively yet.
    client.slowlog(['get', String(slowlogGetLimit)], async function(err,res){ // get the most recent 128 slowlogs (128 is the default)
      if(err){reject(err)}
...
exports.handler = async function (event, context, callback) { //eslint-disable-line

  // cloudwatch logs require you to submit no more than 24hr in a single batch, we cut 60sec off of that as a margin
  const cloudwatchLogCutoffTimestamp = Math.round((new Date().getTime() - (24 * 60 * 60 * 1000)) / 1000 - 60)
  ...
  await asyncForEach(redisTargets, async (redisTarget) => {
    ...
    let slowLogs = null
    try {
      slowLogs = await getSlowlogs(client, cloudwatchLogCutoffTimestamp, slowlogGetLimit)
      console.log("Retrieved "+slowLogs.length+" slowlog entries from "+redisUrl)
    }
    catch {
    ...
    if (logGroups.logGroups.length <= 0) {
      ...
      const logGroupResult = await cloudwatchlogs.createLogGroup(params).promise()
    }
    ...
    if (logStreams.logStreams.length <= 0) {
      ...
      const logStreamResult = await cloudwatchlogs.createLogStream(params).promise()
    ...
    if (slowLogs.length > 0) {
      ...
      if (logStreams.logStreams[0].uploadSequenceToken !== undefined){ params['sequenceToken'] = logStreams.logStreams[0].uploadSequenceToken}
      // put logs
      console.log("Attempting to put "+params.logEvents.length+" slowlogs into "+logGroupName+"/"+redisTarget)
